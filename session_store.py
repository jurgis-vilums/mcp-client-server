...
class InMemorySessionStore(Generic[T]):
    ...
    async def get_or_create(self, session_id: str, factory: Callable[[], T]) -> T:
        async with self._lock:
    ...
    async def set(self, session_id: str, value: T) -> None:
        async with self._lock:
...
class SQLiteMessageHistoryStore:
    ...
    async def list_sessions(self, limit: int = 100, offset: int = 0) -> list[tuple[str, float]]:
